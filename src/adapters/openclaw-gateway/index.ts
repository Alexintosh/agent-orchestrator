// ---------------------------------------------------------------------------
// OpenClaw Gateway Adapter — self-contained port from Paperclip's openclaw-gateway adapter.
// All functionality (execute, testEnvironment, WebSocket protocol, ED25519 device signing)
// is included inline.
// ---------------------------------------------------------------------------
...
const DEFAULT_CLIENT_VERSION = "orchestrator";
...
function resolveApiUrlOverride(value: unknown): string | null {
...
function buildAgentEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
...
function buildWakeText(payload: WakePayload, agentEnv: Record<string, string>): string {
...
function buildStandardPayload(
  ...
  const runtimeServiceIntents = Array.isArray(rawRuntimeIntents)
    ? rawRuntimeIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];
...
// ---------------------------------------------------------------------------
// ED25519 device identity / signing
// ---------------------------------------------------------------------------
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "orchestrator-complete");
    this.ws = null;
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (
        ...
        const failedPairResult = pairResult as { ok: false; reason: string };
        await ctx.onLog(
          "stderr",
...
const agentConfigurationDoc = `# openclaw_gateway agent configuration
...
export default openclawGatewayAdapter;
